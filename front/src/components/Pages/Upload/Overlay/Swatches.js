...
const mapStateToProps = mapDynamicState('uploader: currentCanvasData imageData inputs labels imagesToUpload');
const mapDispatchToProps = mapDynamicDispatch(uploaderState.actions, 'setCurrentCanvasData addCanvasField addCanvasLabel setImageToUpload resetReducer');
/*const mapDispatchToProps = dispatch => ({
  setCurrentCanvasData: currentCanvasData => dispatch(setCurrentCanvasData(currentCanvasData)),
  addCanvasField: field => dispatch(addCanvasField(field)),
  addCanvasLabel: label => dispatch(addCanvasLabel(label)),
  setImageToUpload: images => dispatch(setImageToUpload(images)),
...
class Swatches extends React.Component {
  ...
  componentWillUnmount = () => this.props.resetReducer();

  updateCurrentCanvasData = () => setCurrentCanvasData(this.props.currentCanvasData);
  ...
  uploadHandler = () => {
    let { imagesToUpload, setImageToUpload } = this.props;
    ...
    setImageToUpload({ imagesToUpload });
      ...
      <div style={{color: 'white', top: '80px', textAlign: 'center'}} className="colors color-picker-panel">
        ...
        <div className="panel-row">
          ...
          <Slider
            ...
            min={0.05}
            max={1}
            onChange={this.onSlideChange('alpha')}
          />
          ...
          <Slider
            ...
            min={16}
            max={100}
            onChange={this.onSlideChange('text', true)}
          />

          {
            ...
            <div>
              ...
              <Slider
                ...
                min={10}
                max={100}
                onChange={this.onSlideChange('erase')}
              />
            </div>
          }
          <label htmlFor="inputsLayer">Display text</label>
          <Checkbox
            onClick={this.handleCheckboxChange('inputsLayer')}
            onChange={this.handleCheckboxChange('inputsLayer')}
            checked={inputsLayer}
            name="inputsLayer"
            id="inputsLayer"
          ...
          <Checkbox
            onClick={this.handleCheckboxChange('mainLayer')}
            onChange={this.handleCheckboxChange('mainLayer')}
            checked={mainLayer}
            name="mainLayer"
            id="mainLayer"
          ...
          <br/>
          <Button style={{color: '#fff'}} onClick={this.createInput} >Create a textbox</Button>
          <br/>
          <Button style={{color: '#fff'}} onClick={this.resetCanvas} id="reset">Reset</Button>
          <br/>
          {
              !uploading &&
              <Button style={{color: '#fff'}} onClick={this.uploadHandler}>Upload</Button>
          }
